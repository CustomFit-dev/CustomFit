...
const Form_I = ({ onClose }) => {
  ...
  const handleEnviarCodigo = async () => {
    ...
    try {
      setLoading(true);
      await axios.post('http://localhost:8000/api/enviar_codigo/', {
        correo_electronico: correoElectronico,
      });

      alert('Código enviado al correo electrónico.');
      setShowVerifyModal(true); // Mostrar el modal
    } catch (error) {
      ...
      {showVerifyModal && ReactDOM.createPortal(
        <div className="overlay">
          <div className="form-container">
            <Form correo={correoElectronico} onClose={() => setShowVerifyModal(false)} />
          </div>
        </div>,
        document.body
      )}
